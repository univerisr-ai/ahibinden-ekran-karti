...
# WARP/Residential proxy destegi (Cloudflare TLS+IP eslesmesi icin)
...
# ==========================================
def _fetch_curl_cffi(url: str, headers: dict, label: str, attempt: int) -> str | None:
    """curl_cffi ile tek sayfa cek (Chrome TLS fingerprint)."""
...
def _fetch_drission(url: str, headers: dict, label: str, attempt: int) -> str | None:
    """DrissionPage CDP ile tek sayfa cek (gercek Chrome, Cloudflare uyumlu)."""
    try:
        ...
        co.set_user_agent(headers.get("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Safari/537.36"))
        # Cookie'leri header olarak ekle
        ...
        # Cookie'leri set et
        if cookie_str:
            tab.get("https://www.sahibinden.com")
            time.sleep(2)
        ...
        if "searchResultsItem" in html or "classifiedTitle" in html:
            return html
        return None
    except Exception as e:
        print(f"  [!] DrissionPage hata: {e}{' — ' + label if label else ''} (deneme {attempt})")
        return None


# Hangi fetch motoru kullanilacak?
_USE_DRISSION = os.getenv("SCRAPER_ENGINE", "").lower() == "drission"


def fetch_page(url: str, headers: dict, label: str = "") -> str | None:
    """Tek sayfayı çeker. Varsayilan curl_cffi, SCRAPER_ENGINE=drission ile DrissionPage."""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            if _USE_DRISSION:
                result = _fetch_drission(url, headers, label, attempt)
            else:
                result = _fetch_curl_cffi(url, headers, label, attempt)
            if result:
                return result
        except Exception as e:
...
def run() -> list[dict]:
    ...
    if _PROXY_URL:
        proxy_display = _PROXY_URL.split("@")[-1] if "@" in _PROXY_URL else _PROXY_URL
...
def process_with_ai(listings: list[dict]):
    """Yapay zekaya ilan verilerini gönderir ve filtrelenmiş sonuçları alır."""
    print("\n  [*] Yapay Zeka Analizi Başlatılıyor...")
    print(f"  [*] {len(listings)} adet ilan yapay zekaya gönderiliyor...")
    
    # TODO: Gerçek AI API entegrasyonu (OpenAI, Gemini vb.) buraya eklenecek.
    # Örnek simülasyon:
    import time
    time.sleep(3) 
    print("  [+] Yapay Zeka analizi tamamlandı! Uygun olan ilanlar ayıklandı.")
    print("  [+] Seçilen ilanlar veritabanına ya da web sitenize otomatik olarak aktarıldı/aktarılacak.")
...
def main():
    ...
    if listings:
        ...
        # Yapay Zeka Entegrasyonunu Çağır
        process_with_ai(listings)
